...
const OrderListScreen = () => {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== 'all') {
      q = query(ordersRef, ...baseQueryConstraints, where("status", "==", statusFilter));
    } else {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(order => 
        (order.clientNameSnapshot?.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (order.servicePackage?.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  const renderOrderItem = ({ item }) => (
    <List.Item
      ...
      description={`Order Date: ${item.orderDate?.toDate ? format(item.orderDate.toDate(), 'PP') : 'N/A'}`}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            ...
            {ORDER_STATUSES.map(status => (
                <Chip 
                    ...
                    style={[styles.chip, {backgroundColor: getStatusColor(status)}, statusFilter === status && styles.chipSelected]}
                    textStyle={{color: '#fff'}}
                >
                    {status.replace('-', ' ')}
                </Chip>
